...
import static pe.edu.unamba.academic.shared.utils.exceptions.ApiErrorResponse.*;
...
class ApiExceptionHandler extends ResponseEntityExceptionHandler{
    ...
    ResponseEntity<ApiErrorResponse> handleDataIntegrityViolationException(DataIntegrityViolationException exception) {
        if (exception.getCause() instanceof ConstraintViolationException) {
            ConstraintViolationException constraintViolationException = (ConstraintViolationException) exception.getCause();
            List<ApiError> errors = constraintViolationException.getConstraintViolations()
                    .stream()
            ...
            return conflict(errors, "Duplicidad de datos - " + constraintViolationException.getMessage());
        }
        return internalServerError("Error de integridad de datos");
    }
    ...
    ResponseEntity<ApiErrorResponse> handleMethodArgumentTypeMismatchException(MethodArgumentTypeMismatchException exception) {
        return badRequest("Tipo de argumento '" + exception.getName() + "' no válido");
    }
